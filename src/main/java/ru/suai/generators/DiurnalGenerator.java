...
public class DiurnalGenerator {
    ...
    /**
     * Intensity (lambda) for Poisson generation.
     */
     ...
     * @param intensity intensity for Poisson thread
     */
    public DiurnalGenerator(HashMap<modulation, Double> modulation, HashMap<distribution, String> distribution, double intensity) {
    ...
    public double getValue(double x) {
        ...
        switch (this.distributionType) {
            case POISSON_DISTRIBUTION_TYPE:
                stochasticNumber = (1 / this.intensity) * Math.log(this.randomGenerator.nextDouble());
                break;
        ...
        switch (this.shapeType) {
            case Predictor.LINEAR_FUNCTION_TYPE:
                base = (Double)this.artificialGenerator.getLinearValue(x);
                break;
            case Predictor.DEGREE_FUNCTION_TYPE:
                base = (Double)this.artificialGenerator.getDegreeValue(x);
                break;
            case Predictor.EXPONENTIAL_FUNCTION_TYPE:
                base = (Double)this.artificialGenerator.getExponentialValue(x);
                break;
        }

        // A * sin((2 * Pi) / T) * x + Fi
        return (base + stochasticNumber + (this.amplitude * Math.sin((2 * Math.PI / (this.period)) * x + this.phase)));
    }
}
